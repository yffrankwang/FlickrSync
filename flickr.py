...
class Photo(object):
	...
	def __init__(self, id, owner=None,\
				 datetaken=None, dateuploaded=None, lastupdate=None, mdate=-1, fsize=0,\
				 ...
				 isfavorite=None, server=None, license=None, rotation=None, url=None, media=None):
		...
		self.takengranularity = None
		self.permcomment = None
		self.permaddmeta = None
	...
	def loadProperties(self):
		...
		self.rotation = photo.rotation

		owner = photo.owner
		self.owner = User(owner.nsid, username=owner.username,\
						  realname=owner.realname,\
						  location=owner.location)

		self.title = photo.title.text
		self.description = photo.description.text
		self.ispublic = photo.visibility.ispublic
		self.isfriend = photo.visibility.isfriend
		self.isfamily = photo.visibility.isfamily

		self.dateposted = photo.dates.posted
		self.datetaken = photo.dates.taken
		self.takengranularity = photo.dates.takengranularity

		self.cancomment = photo.editability.cancomment
		self.canaddmeta = photo.editability.canaddmeta
		self.comments = photo.comments.text

		try:
			self.permcomment = photo.permissions.permcomment
			self.permaddmeta = photo.permissions.permaddmeta
		...
		if hasattr(photo.tags, "tag"):
			if isinstance(photo.tags.tag, list):
				self.tags = [Tag(tag.id, User(tag.author), tag.raw, tag.text) \
	...
	def __str__(self):
		return '<Flickr Photo %s: %s>' % (self.id, self.title)
...
class Photoset(object):
	...
	def __init__(self, id, title, primary, photos=0, videos=0, description='', \
				 secret='', server=''):
		self.__id = id
		self.__title = title
	...
	def __len__(self):
		return self.__photos + self.__videos

	def __str__(self):
		return '<Flickr Photoset %s: %s>' % (self.id, self.title)
...
class User(object):
	...
	def _print_progress(self, head, page, pages):
		if pages == 0:
			page = 0
		sys.stdout.write("\r%s ... %d/%d" % (head, page, pages))
		sys.stdout.flush()
	...
	def getPhotos(self):
		...
		pages = 1
		while page <= pages:
			data = _doget(method, user_id=self.id, extras="description, media, tags, url_o", page=page, per_page=1000)

			photos = data.rsp.photos
			pages = int(photos.pages)

			self._print_progress(method, page, pages)

			if not getattr(photos, 'photo', None):
				break
			
			if isinstance(photos.photo, list):
				for photo in photos.photo:
					sets.append(_parse_photo(photo))
			else:
				photo = photos.photo
				sets.append(_parse_photo(photo))
			page += 1
...
def _prepare_params(method, params):
	_convert_params(params)
	if method:
		params['method'] = method

	if USER_TOKEN:
		params['auth_token'] = USER_TOKEN
	return params
...
def _sign_call(data):
	...
	tohash = API_SECRET + "api_key" + API_KEY + args
	return hashlib.md5(tohash).hexdigest()

def _url_payload(data):
	sig = _sign_call(data)
	data["api_key"] = API_KEY
	data["api_sig"] = sig
	payload = urllib.urlencode(data)
	#'&'.join(key+'='+value for key, value in data.iteritems())
	return payload
...
"""
def _url_gen(base, data):
	query = _url_payload(data)
	return base + "?" + query
...
def _doget(method, auth=False, **params):
	...
	params = _prepare_params(method, params)
	url = _url_gen('%s%s/' % (HOST, API), params)

	#another useful debug print statement
	logging.debug(url)

	return _get_data(minidom.parse(urllib.urlopen(url)))
...
def _dopost(method, auth=False, **params):
	...
	params = _prepare_params(method, params)
	url = '%s%s/' % (HOST, API)
	payload = _url_payload(params)

	#another useful debug print statement
	logging.debug(url + '?' + payload)

	return _get_data(minidom.parse(urllib.urlopen(url, payload)))
	...
	for item in paramaters:
		for chocolate in params.items():
			if item == chocolate[0]:
				...
				api_string.append(chocolate[1].decode('UTF-8'))
		if item == 'method':
...
def _parse_photo(photo):
	...
	url = None
	if hasattr(photo, 'url_o'):
		url = photo.url_o
	media = None
	if hasattr(photo, 'media'):
		media = photo.media

	p = Photo(photo.id, owner=owner, \
			title=photo.title, \
			...
			secret=photo.secret, \
			server=photo.server, \
			description=description, url=url, media=media)
	return p
